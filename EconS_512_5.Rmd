...
tt<-c()
for (i in 1:dim(data)[1]){
  if (data[i,"year"]==1987)tt[i]<-0
  else if (data[i,"year"]==1988)tt[i]<-1
  else tt[i]<-2
}
...
####Codes following this is needed to work on###########
...
yin<-data[,"hrsemp"]
y.temp.d<-yd*dum.f[,1]
y.temp.in<-yin*dum.f[,1]
xr.temp<-xr*dum.f[,1]
beta.d<-solve(t(xr.temp)%*%xr.temp)%*%(t(xr.temp)%*%y.temp.d)
beta.in<-solve(t(xr.temp)%*%xr.temp)%*%(t(xr.temp)%*%y.temp.in)
res.d[1:length(which(data[,"fcode"]==fac[1]))]<-y.temp.d-xr.temp%*%beta.d
res.in[1:length(which(data[,"fcode"]==fac[1]))]<-y.temp.in-xr.temp%*%beta.in
#######This Part of the code needs attention in future#########
